...
pub (in super) unsafe fn get_global_ptr(offset: usize) -> *const u8 {
    let global_addr = offset + get_module_start();

    global_addr as *const u8
}

pub (in super) unsafe fn get_global_ptr_mut(offset: usize) -> *mut u8 {
    let global_addr = offset + get_module_start();

    global_addr as *mut u8
}

pub (in super) unsafe fn transmute_offset<T>(offset: usize) -> &'static T {
