...
pub unsafe fn relocate_self(mod0: &Mod0) {
    let mut current_elf_dyn_ptr = get_global_ptr(mod0.dynamic_offset as usize);
    ...
    loop {
        ...
        match DynTags::from(current_elf_dyn.tag) {
            DynTags::Null => break,
            DynTags::RelocationOffset => temp_relocation_info.offset = Some(current_elf_dyn.value as usize),
            ...
            DynTags::RelocationAddendOffset => temp_relocation_addend_info.offset = Some(current_elf_dyn.value as usize),
            DynTags::RelocationAddendCount => temp_relocation_addend_info.count = Some(current_elf_dyn.value as usize),
    ...
    if let Some(relocation_addend_info) = RelocationInfo::load_from_temporary::<RelocationAddendEntry>(&temp_relocation_addend_info) {
        for relocation in relocation_addend_info.entries::<RelocationAddendEntry>() {
            if RelocationType::AArch64Relative == RelocationType::from(relocation.info.relocation_type) {
                *(get_global_ptr_mut(relocation.offset as usize) as *mut usize) = get_module_start() + relocation.addend as usize;
            }
        }
    }
}