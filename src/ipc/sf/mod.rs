use core::{fmt::Write, iter::zip, panic::Location, prelude::v1};
...
use crate::{
  ...
  svc::{self, output_debug_bytes, output_debug_string},
...
impl<'a, D: Copy + AsBytes> RequestContext<'a, D> {
  ...
  pub fn cause_crash_on_request(mut self) -> Self {
    self.crash_on_request = true;
    self
  }
  ...
  pub fn add_out_autoselect<T>(mut self, mode: Mode, data: &'a mut [T]) -> Self {
    self
      .hipc_receive_statics
      .push(hipc::ReceiveStatic::new(0, 0))
      .unwrap();
    self
      .hipc_receive_buffers
      .push(hipc::Buffer::new(
        data.len() as u64,
        data.as_ptr() as u64,
        mode,
...
{
  output_debug_string(Location::caller().file());
  write_request(&service, cmif::CommandType::Control, request_context)?;
  if request_context.crash_on_request {
    tls::dump();
    svc::panic(0x34, 0, 0)
  }
  service
...
where
  ResFn: FnOnce(&RequestContext<'_, Req>, Response<'_>) -> Result<Out, Error>,
{
  let mut str: String<4096> = String::new();
  write!(&mut str, "Request {} from: {}", request_context.command, Location::caller()).unwrap();
  ...
  let response = read_response(service)?;
  str.clear();
  write!(&mut str, "Response {} for: {}", request_context.command, Location::caller()).unwrap();
  output_debug_string(str.as_str());
  response_handler(request_context, response)
}
...
fn read_response<'a, Srv>(service: &Service<Srv>) -> Result<Response<'a>, Error> {
  ...
  output_debug_string("reading response");

  let header: hipc::Header = reader.read()?;
  output_debug_string("read hipc header");
  ...
  if header.has_special_header() {
    output_debug_string("special header");
  ...
  output_debug_string("reading send statics");

  let hipc_send_statics = reader.read_vec(header.send_static_count() as usize)?;

  reader.align_to(16);

  output_debug_string("reading data words");
  ...
  output_debug_string("reading out header");
  let cmif_header = reader.read::<cmif::OutHeader>()?;
  data_words -= 4;

  output_debug_string("sfco");
  output_debug_bytes(cmif_header.magic().as_bytes());

  if cmif_header.result().is_failure() {
    return Err(Error::ResponseError(cmif_header.result()));
  }

  output_debug_string("reading data");
  let data = reader.read_slice(data_words * 4)?;

  output_debug_string("fully read my nuts out yo mouth");
