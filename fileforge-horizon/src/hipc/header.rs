...
impl Header {
  ...
  pub fn set_send_static_count(&mut self, send_static_count: u8) {
    if send_static_count > 0xF {
      panic!(
        "send_static_count too high! max is 0xF, got: {}",
        send_static_count
      );
    }
  ...
  pub fn set_send_buffer_count(&mut self, send_buffer_count: u8) {
    if send_buffer_count > 0xF {
      panic!(
        "send_buffer_count too high! max is 0xF, got: {}",
        send_buffer_count
      );
    }
  ...
  pub fn set_receive_buffer_count(&mut self, receive_buffer_count: u8) {
    if receive_buffer_count > 0xF {
      panic!(
        "receive_buffer_count too high! max is 0xF, got: {}",
  ...
  pub fn set_exchange_buffer_count(&mut self, exchange_buffer_count: u8) {
    if exchange_buffer_count > 0xF {
      panic!(
        "exchange_buffer_count too high! max is 0xF, got: {}",
        exchange_buffer_count
      );
    }
  ...
  pub fn set_data_words(&mut self, data_words: u16) {
    if data_words > 0x3FF {
      panic!(
        "data_words too high! max is 0x3FF (1kb), got: {}",
        data_words
      );
    }
  ...
  pub fn with_request_type(mut self, request_type: u16) -> Self {
    self.set_request_type(request_type);
    self
  }
  pub fn with_send_static_count(mut self, send_static_count: u8) -> Self {
    self.set_send_static_count(send_static_count);
    self
  }
  pub fn with_send_buffer_count(mut self, send_buffer_count: u8) -> Self {
    self.set_send_buffer_count(send_buffer_count);
    self
  }
  pub fn with_receive_buffer_count(mut self, receive_buffer_count: u8) -> Self {
    self.set_receive_buffer_count(receive_buffer_count);
    self
  }
  pub fn with_exchange_buffer_count(mut self, exchange_buffer_count: u8) -> Self {
    self.set_exchange_buffer_count(exchange_buffer_count);
    self
  }
  pub fn with_data_words(mut self, data_words: u16) -> Self {
    self.set_data_words(data_words);
    self
  }
  pub fn with_special_header(mut self, special_header: bool) -> Self {
    self.set_special_header(special_header);
    self
  }
}
...
impl<const NODE_NAME_SIZE: usize> fileforge_lib::error::Error<NODE_NAME_SIZE> for HeaderReadError {
...
impl<'pool, const DIAGNOSTIC_NODE_NAME_SIZE: usize> FixedSizeReadable<'pool, DIAGNOSTIC_NODE_NAME_SIZE, 8> for Header {
  type Argument = ();
  ...
  fn read<RP: Provider>(reader: &mut Reader<'pool, DIAGNOSTIC_NODE_NAME_SIZE, RP>, _: Self::Argument) -> Result<Self, ParseError<'pool, Self::Error, RP::ReadError, DIAGNOSTIC_NODE_NAME_SIZE>> {
    Ok(reader.with_dyn_bytes(Some(8), "Contents", |bytes| {
